...
class _AddOrEditEventFormState extends State<AddOrEditEventForm> {
            ...
            children: [
              ...
              Expanded(
                child: DateTimeSelectorFormField(
                  ...
                  decoration: AppConstants.inputDecoration.copyWith(
                    labelText: "End Date",
                    // Add a disabled style when recurring is true
                    filled: _isRecurring,
                    fillColor: _isRecurring ? Colors.grey[200] : null,
                  ),
                  onSelect: _isRecurring
                      ? null
                      : (date) {
                          if (date.withoutTime.withoutTime
  ...
  void _createEvent() {
    ...
    if (_selectedFrequency != null && _selectedFrequency != RepeatFrequency.doNotRepeat) {
    ...
    final DateTime eventEndDate = (_selectedFrequency == RepeatFrequency.doNotRepeat)
        ? _endDate
  ...
  void _setDefaults() {
    ...
    if (event.recurrenceSettings != null && 
      ...
      if (_selectedRecurrenceEnd == RecurrenceEnd.after) {
        _occurrenceController.text = (event.recurrenceSettings!.occurrences ?? 1).toString();
      }
      
      // Clear weekdays selection and then set the selected days
      _selectedDays = List.filled(7, false);
      event.recurrenceSettings!.weekdays.forEach((index) => _selectedDays[index] = true);
    } else {
      _isRecurring = false;
      _selectedFrequency = RepeatFrequency.doNotRepeat;
      updateWeekdaysSelection();
